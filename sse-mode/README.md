...
### 📦 步驟二：安裝必要套件
> 💡 對 SSE 模式的 MCP Server 來說，只有 `mcp` 是必要的。`httpx` 則是範例中用來處理網路請求的額外套件。
```shell
uv add mcp httpx
```
...
啟動 SSE Server，然後再用 MCP Inspector 進行連線測試。

1. 啟動 SSE Server，並指定運行 `weather.py`
    ```shell
    python weather.py
    ```
...
4. 確認 Inspector 啟動資訊，Console 會顯示以下資訊，並自動打開瀏覽器：
    ...
    ```
5. 進行連線：選擇 SSE Mode → URL 輸入 http://localhost:8000/sse → 選擇 Vai Proxy → 點 Connect
